...
export default function TournamentView({ tournament }) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      {/* Standings Table (Only A or B) */}
      {activeGroup !== 'KO' && (
        <div className="glass-panel overflow-hidden">
          <div className="px-4 py-3 bg-slate-800/50 border-b border-slate-700/50 flex justify-between items-center">
            <h3 className="text-sm font-bold uppercase tracking-wider text-white">Tabla de Posiciones</h3>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-400 uppercase bg-slate-900/50">
                <tr>
                  ...
                  <th className="px-4 py-2 font-black text-emerald-400 text-center">PTS</th>
                </tr>
              </thead>
              <tbody>
                {currentStandings.map((player, idx) => (
                  <tr key={player.name} className={`border-b border-slate-800 last:border-0 ${idx < 2 ? 'bg-emerald-500/5' : ''}`}>
                    <td className="px-4 py-3 font-medium flex items-center gap-2">
                      <span className={`w-5 text-center text-xs font-black ${idx < 2 ? 'text-emerald-400' : 'text-slate-600'}`}>{idx + 1}</span>
                      {player.name}
                    </td>
                    <td className="px-2 py-3 text-center text-slate-400">{player.played}</td>
                    <td className="px-2 py-3 text-center text-slate-400">{player.goalDifference > 0 ? `+${player.goalDifference}` : player.goalDifference}</td>
                    <td className="px-4 py-3 text-center font-black text-white">{player.points}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Matches List (Only A or B) */}
      {activeGroup !== 'KO' && (
        <div className="glass-panel p-4 mb-20">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-4">Fixture - Grupo {activeGroup}</h3>
          <div className="space-y-3">
            ...
            {currentGroupMatches.map((match) => (
              <div key={match.id} className={`flex items-center justify-between p-3 rounded-lg border ${match.closed ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-slate-800/40 border-slate-700/50'}`}>
                
                <div className="flex-1 text-right font-medium text-sm pr-3 truncate max-w-[40%]">
                  {match.player1}
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <input
                    ...
                    max="20"
                    value={match.score1 ?? ''}
                    onChange={(e) => handleScoreChange(match, e.target.value, match.score2)}
                    disabled={match.closed}
                    className="w-10 h-10 bg-slate-900 border border-slate-600 rounded text-center font-black text-lg focus:outline-none focus:border-emerald-400 disabled:opacity-100 disabled:text-emerald-400"
                  />
                  ...
                  <input
                    ...
                    max="20"
                    value={match.score2 ?? ''}
                    onChange={(e) => handleScoreChange(match, match.score1, e.target.value)}
                    disabled={match.closed}
                    className="w-10 h-10 bg-slate-900 border border-slate-600 rounded text-center font-black text-lg focus:outline-none focus:border-emerald-400 disabled:opacity-100 disabled:text-emerald-400"
                  />
                </div>

                <div className="flex-1 text-left font-medium text-sm pl-3 truncate max-w-[40%]">
                  {match.player2}
                </div>
      ...
      {/* Knockout Stage */}
      {activeGroup === 'KO' && (
        <div className="glass-panel p-4 mb-20 bg-emerald-950/20">
            ...
            {knockoutList.map((match) => (
              <div key={match.id} className={`p-4 rounded-xl border ${match.id === 'final' ? 'border-amber-500/50 bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'border-slate-700/50 bg-slate-800/40'}`}>
                ...
                <div className="flex items-center justify-between">
                  <div className={`flex-1 text-right font-bold text-sm pr-3 truncate max-w-[40%] ${match.score1 > match.score2 ? 'text-white' : 'text-slate-400'}`}>
                    {match.p1}
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <input
                      ...
                      max="20"
                      value={match.score1 ?? ''}
                      onChange={(e) => handleKnockoutScoreChange(match.id, e.target.value, match.score2)}
                      disabled={match.closed}
                      className={`w-10 h-10 border rounded text-center font-black text-lg focus:outline-none focus:border-emerald-400 disabled:opacity-100 ${match.id === 'final' ? 'bg-slate-900 border-amber-600/50 text-amber-100' : 'bg-slate-900 border-slate-600 text-white'}`}
                    />
                    <span className="text-slate-500 font-black">-</span>
                    <input
                      type="number"
                      ...
                      max="20"
                      value={match.score2 ?? ''}
                      onChange={(e) => handleKnockoutScoreChange(match.id, match.score1, e.target.value)}
                      disabled={match.closed}
                      className={`w-10 h-10 border rounded text-center font-black text-lg focus:outline-none focus:border-emerald-400 disabled:opacity-100 ${match.id === 'final' ? 'bg-slate-900 border-amber-600/50 text-amber-100' : 'bg-slate-900 border-slate-600 text-white'}`}
                    />
                  </div>

                  <div className={`flex-1 text-left font-bold text-sm pl-3 truncate max-w-[40%] ${match.score2 > match.score1 ? 'text-white' : 'text-slate-400'}`}>
                    {match.p2}
                  </div>
                </div>
              </div>
